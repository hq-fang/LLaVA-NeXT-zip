    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                # num_added_tokens = tokenizer.add_tokens(["<LINE_START>", "<LINE_END>"] + [f"<LINE_{num}>" for num in range(257)])
            ...
            model.lm_head.weight = torch.nn.Parameter(torch.empty(len(tokenizer), tokem_dim, device=model.device, dtype=model.dtype))
            model.model.embed_tokens.weight = torch.nn.Parameter(torch.empty(len(tokenizer), tokem_dim, device=model.device, dtype=model.dtype))
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        llava_cfg = LlavaQwenConfig.from_pretrained(model_path)
                        # print("LlavaQwenConfig:", llava_cfg)
                        # print("Config in JSON format:")
                        # print(llava_cfg.to_json_string())
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        # print("LlavaQwenConfig:", llava_cfg)
