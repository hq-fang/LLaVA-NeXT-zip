...
# # Only launch debugpy for the main (or local rank 0) process
...
#         # --- New: Annotate the image if line_mod is provided ---
...
#             line_mod = self.list_data_dict[i].get("line_mod", None)
#             if type(image_file) is list:
#                 image = [self.process_image(f, line_mod=line_mod) for f in image_file]
#                 # Handling multi images
#                 # overwrite to process with simple pad 
#                 if len(image_file) > 1:
#                     image = [self.process_image(f, overwrite_image_aspect_ratio="pad", line_mod=line_mod) for f in image_file]
#                     image = [[im[0], im[1], "image"] for im in image]
#             else:
#                 image = [self.process_image(image_file, line_mod=line_mod)]
...
#     print("len tokenizer", len(tokenizer))
#     num_added_tokens = tokenizer.add_tokens(["<ACTION_START>", "<ACTION_END>"] + [f"<ACTION_{num}>" for num in range(256)])
#     print("Number of added tokens: ", num_added_tokens)
#     print("len tokenizer", len(tokenizer))
...
#             ]
#         ):
#             print("n: ", n, "p.shape: ", p.weight.shape, "True")
...
from llava.train.io import get_bytes_range
# For Google Cloud Storage (if using GCS-specific helpers)
from google.cloud import storage
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None, line_mod=None):
        ...
        try:
            # Construct the full remote path
            image_path = f"{image_folder.rstrip('/')}/{image_file.lstrip('/')}"
            # Open the image from the remote bucket using fsspec
            ...
            print(f"Failed to open image {image_file} from {image_path}. Exception:", exn)
            raise exn
    # def process_image(self, image_file, overwrite_image_aspect_ratio=None, line_mod=None):
    #     print(f"[DEBUG] process_image started for: {image_file}")
    #     image_folder = self.data_args.image_folder  # e.g., "gs://vision-jiafeid/path/to/images"
    #     processor = self.data_args.image_processor
    #     # Construct the full remote path
    ...
    #     remote_path = f"{image_folder.rstrip('/')}/{image_file.lstrip('/')}"
    #     print("[DEBUG] Bucket listing:", fs.ls("vision-jiafeid"))

    #     print(f"[DEBUG] Attempting to open remote_path: {remote_path}")
    #     try:
    #         # with fsspec.open(remote_path, "rb", token="google_default") as f:
    #         with fs.open(remote_path, "rb") as f:
    #             print("[DEBUG] fsspec.open() succeeded, file object obtained.")
    #             # Read a small chunk to confirm it's working
    #             chunk = f.read(100)
    #             print(f"[DEBUG] First 100 bytes: {chunk[:50]}...")  # show first 50 bytes for brevity
    #             image = Image.open(f).convert("RGB")
    #             print(f"[DEBUG] Image opened. Size: {image.size}")
    #     except Exception as exn:
    #         print(f"[DEBUG] Failed to open image {image_file} from {remote_path}. Exception: {exn}")
    #         raise exn
